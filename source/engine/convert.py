...
# convert input to reference coordinate system
def run_convert(inputpath,refpath,outputpath):
  ...
  for item in data:
    ...
    # set z-offset (minimum DSM value)
    gt.z_off(meta['Minimum'])

    # compute image to local x/y transformation
    if item['region_pc']:
      tmp = item['region_pc']
    else:
      G = meta['GeoTransform']
    ...
    fileout = os.path.join(outputpath,item['name']+'.json')
...
def main(args=None):
  ...
  parser.add_argument('-i', '--input', dest='inputpath',
      help='Input directory for conversion', required=True)
